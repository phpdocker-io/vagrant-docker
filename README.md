...
`config.yaml` file on the root of this project. You have a template you can peruse at
[config.yaml.dist](config.yaml.dist).
...
* `services` _(optional)_: list of service definitions to set up as hosts and on supervisor on the VM. See below for
  object structure:
    ...
    * `git_repo` _(optional)_: if provided, we'll git-clone this repo in the [projects](projects) folder at the location
      of
...
Each service defined in `config.yaml`  is set up as a supervisor program service. You can have a look at the
...
`config.yaml` file, as well as hosts entries on your host computer to match and SSL certificates (more on those
...
If your service exposes an SSL endpoint, you must indicate `is_ssl: true` on the gateway config. It will not verify SSL
certs to allow for self-signed certificates there.
...
### I've added a new service to my config.yaml, now what?
...
You might want to do this only on your dev container, which you can achieve by using multi stages:
